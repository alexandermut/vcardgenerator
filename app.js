...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function refreshUI({ validateTouched = false } = {}) {
        ...
        updateVCFPreview(vcfPreviewElement, data, allErrors, vcf);
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (Object.keys(allErrors).length > 0) {
            ...
            feedback.classList.remove('success');
            clearQRCode(qrContainer, qrDownloadButton);
            updateVCFPreview(vcfPreviewElement, data, allErrors, '');
            return;
        }
...
function updateVCFPreview(element, data, errors, vcfString) {
    if (!element) return;

    const hasNames = Boolean(data.firstName) && Boolean(data.lastName);
    if (!hasNames) {
        element.textContent = 'Fülle mindestens Vor- und Nachname aus, um die Vorschau zu sehen.';
        element.classList.add('placeholder');
        return;
    }

    const hasErrors = Object.keys(errors).length > 0;
    if (hasErrors || !vcfString) {
        element.textContent = 'Bitte korrigiere die markierten Felder, um die vCard-Vorschau zu sehen.';
        element.classList.add('placeholder');
        return;
    }

    element.textContent = vcfString;
